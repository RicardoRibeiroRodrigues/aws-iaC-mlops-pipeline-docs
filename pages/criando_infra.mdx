...
export AWS_SECRET_ACCESS_KEY=<SUA SECRET ACESS KEY>
```
...
- mirror_url_build.
- mirror_url_deploy.
- mirror_git_password (essa deve não sai no terminal, fica em terraform.tfstate).
...
<Tabs items={['Forçar o push', 'Fazer commit']}>
  <Tab>
    (COLOCAR IMAGEM DE COMO Forçar O PUSH)
  </Tab>
  <Tab>
    Altere alguma coisa para seu objetivo nos repositórios do gitlab, dê commit e push, você deve ver a pipeline iniciando em alguns minutos.
  </Tab>
</Tabs>
...
<Callout type="warning" emoji="⚠️">
    ...
    **É esperado também, que a primeira execução da pipeline de deploy falhe**, já que o modelo da etapa de build não deve ter sido registrado ainda, mas ao ter o código da pipeline de deploy no CodeCommit e o build ter sucesso, o próximo deploy dará certo também!
</Callout>

Paciência! é esperado que a pipeline demore um pouco, afinal, estamos criando a infra para cada etapa, e treinando um modelo, isso leva tempo!
